...
export async function detectAvailableBackend(): Promise<string | null> {
  ...
  const possibleUrls = (window as any).POSSIBLE_BACKEND_URLS || [
    `http://${CONFIG_DEFAULTS.PRODUCTION_HOST}:${CONFIG_DEFAULTS.BACKEND_PORT}`,
...
export function getBackendUrl(): string {
  ...
  // 回退到默认值
  return `http://${CONFIG_DEFAULTS.PRODUCTION_HOST}:${CONFIG_DEFAULTS.BACKEND_PORT}`;
}
