...
import { DEFAULT_CONFIG, DEVELOPMENT_CONFIG, PRODUCTION_CONFIG } from '../config/default.ts'
...
export class ConfigManager {
  ...
  private readEnvConfig(): Partial<AppConfig> {
    ...
    if (import.meta.env.VITE_API_URL) {
      env.apiUrl = import.meta.env.VITE_API_URL
    }
    
    if (import.meta.env.VITE_API_TIMEOUT) {
      env.apiTimeout = parseInt(import.meta.env.VITE_API_TIMEOUT, 10)
    }

    // 环境配置
    if (import.meta.env.MODE) {
      env.environment = import.meta.env.MODE as AppConfig['environment']
    }

    // 应用信息
    if (import.meta.env.VITE_APP_NAME) {
      env.appName = import.meta.env.VITE_APP_NAME
    }

    if (import.meta.env.VITE_APP_VERSION) {
      env.version = import.meta.env.VITE_APP_VERSION
    }

    // 功能开关
    const features: Partial<AppConfig['features']> = {}
    if (import.meta.env.VITE_ENABLE_NOTIFICATIONS) {
      features.enableNotifications = import.meta.env.VITE_ENABLE_NOTIFICATIONS === 'true'
    }
    if (import.meta.env.VITE_ENABLE_SSE) {
      features.enableSSE = import.meta.env.VITE_ENABLE_SSE === 'true'
    }
    if (import.meta.env.VITE_ENABLE_OFFLINE_MODE) {
      features.enableOfflineMode = import.meta.env.VITE_ENABLE_OFFLINE_MODE === 'true'
    }
    ...
    if (import.meta.env.VITE_THEME) {
      ui.theme = import.meta.env.VITE_THEME as AppConfig['ui']['theme']
    }
    if (import.meta.env.VITE_LANGUAGE) {
      ui.language = import.meta.env.VITE_LANGUAGE as AppConfig['ui']['language']
    }
    if (import.meta.env.VITE_SIDEBAR_WIDTH) {
      ui.sidebarWidth = parseInt(import.meta.env.VITE_SIDEBAR_WIDTH, 10)
    }
