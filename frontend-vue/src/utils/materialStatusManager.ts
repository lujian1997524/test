  ...
  if (currentProject && currentProject.status !== newStatus) {
    ...
    try {
      ...
      if (newStatus === 'completed') {
        try {
          const response = await fetch(`http://110.40.71.83:35001/api/drawings/archive-project/${projectId}`, {
            method: 'POST',
            headers: {
...
export const updateMaterialStatusShared = async (
  ...
  try {
    ...
    if (newStatus === 'empty') {
      // 如果切换到空白状态，删除现有材料记录
      if (existingMaterial) {
        const deleteResponse = await fetch(`http://110.40.71.83:35001/api/materials/${existingMaterial.id}`, {
          method: 'DELETE',
          headers: {
    ...
    if (existingMaterial) {
      ...
      const response = await fetch(`http://110.40.71.83:35001/api/materials/${existingMaterial.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      const createResponse = await fetch('http://110.40.71.83:35001/api/materials', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
