...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({
  ...
  const fetchProjectDetail = async () => {
    try {
      ...
      // 获取厚度规格
      const specsResponse = await fetch('/api/thickness-specs', {
      ...
      // 获取材料
      const materialsResponse = await fetch(`/api/materials/project/${projectId}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });

      // 获取图纸
      const drawingsResponse = await fetch(`/api/drawings/project/${projectId}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
